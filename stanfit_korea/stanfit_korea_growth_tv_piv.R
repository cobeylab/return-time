...
data_fit <- data_korea_ari_scaled %>%
  filter(key=="Parainfluenza virus")
...
stanfit_korea_growth_tv_piv <- sampling(model,
                                         data = standata,
                                         seed=101,
                                         chain=4,
                                         cores=4,
...
check_hmc_diagnostics(stanfit_korea_growth_tv_piv)
get_num_divergent(stanfit_korea_growth_tv_piv)
get_num_max_treedepth(stanfit_korea_growth_tv_piv)
get_low_bfmi_chains(stanfit_korea_growth_tv_piv)
get_bfmi(stanfit_korea_growth_tv_piv)

save("stanfit_korea_growth_tv_piv", file="stanfit_korea_growth_tv_piv.rda")

ss <- summary(stanfit_korea_growth_tv_piv)

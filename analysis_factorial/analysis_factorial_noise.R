...
for (i in 1:nrow(paramdata)) {
  ...
  ss <- simulate_sirs_stochastic(R0=R0, delta=delta, theta=0, N=N, sigma_env=0, rho_env=0,
...
analysis_factorial_noise <- reslist %>%
  bind_rows

save("analysis_factorial_noise", 
     file="analysis_factorial_noise.rda")

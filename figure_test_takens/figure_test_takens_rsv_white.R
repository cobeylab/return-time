...
lfit_takens1 <- lm(log(dist)~time, data=filter(dist_takens1, time>=maxt_takens1, time<=maxt_takens1+1/ee1))
...
g4 <- ggplot(filter(dist_takens1, time >= 2020)) +
  ...
  geom_function(fun=function(x) exp(predict(lfit_takens1)[1]) * exp(-ee1*(x-maxt_takens1)),
                lwd=0.7, lty=3) +
  geom_smooth(data=filter(dist_takens1, time>=maxt_takens1, time<=maxt_takens1+1/ee1), aes(time, dist), method="lm", col="#224B95") +
  scale_x_continuous("Year") +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
  coord_cartesian(ylim=c(1e-5, 10)) +
  theme(
...
lfit_takens12 <- lm(log(dist)~time, data=filter(dist_takens12, time>=maxt_takens12, time<=maxt_takens12+1/ee1))
...
g6 <- ggplot(filter(dist_takens12, time >= 2020)) +
  ...
  geom_function(fun=function(x) exp(predict(lfit_takens12)[1]) * exp(-ee1*(x-maxt_takens12)),
                lwd=0.7, lty=3) +
  geom_smooth(data=filter(dist_takens12, time>=maxt_takens12, time<=maxt_takens12+1/ee1), aes(time, dist), method="lm", col="#224B95") +
  scale_x_continuous("Year") +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
  coord_cartesian(ylim=c(1e-5, 10)) +
  theme(
...
lfit_takens2 <- lm(log(dist)~time, data=filter(dist_takens2, time>=maxt_takens2, time<=maxt_takens2+1/ee2))
...
g10 <- ggplot(filter(dist_takens2, time >= 2020)) +
  ...
  geom_function(fun=function(x) exp(predict(lfit_takens2)[1]) * exp(-ee2*(x-maxt_takens2)),
                lwd=0.7, lty=3) +
  geom_smooth(data=filter(dist_takens2, time>=maxt_takens2, time<=maxt_takens2+1/ee2), aes(time, dist), method="lm", col="#224B95") +
  scale_x_continuous("Year") +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
  coord_cartesian(ylim=c(1e-5, 10)) +
  theme(
...
lfit_takens22 <- lm(log(dist)~time, data=filter(dist_takens22, time>=maxt_takens22, time<=maxt_takens22+1/ee2))
...
g12 <- ggplot(filter(dist_takens22, time >= 2020)) +
  ...
  geom_function(fun=function(x) exp(predict(lfit_takens22)[1]) * exp(-ee2*(x-maxt_takens22)),
                lwd=0.7, lty=3) +
  geom_smooth(data=filter(dist_takens22, time>=maxt_takens22, time<=maxt_takens22+1/ee2), aes(time, dist), method="lm", col="#224B95") +
  scale_x_continuous("Year") +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
  coord_cartesian(ylim=c(1e-5, 10)) +
  theme(
...
ggsave("figure_test_takens_rsv_white.pdf", gcomb, width=12, height=16)

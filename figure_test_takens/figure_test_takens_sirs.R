...
mat_perturb_takens <- takens(log(ss_perturb$I), d=2, tau=tau)
mat_unperturb_takens <- takens(log(ss_unperturb$I), d=2, tau=tau)
...
mat_perturb_takens2 <- takens(log(ss_perturb$I), d=3, tau=tau)
mat_unperturb_takens2 <- takens(log(ss_unperturb$I), d=3, tau=tau)
...
lfit_takens <- lm(log(dist)~time, data=filter(dist_takens, time>=maxt_takens))

g4 <- ggplot(filter(dist_takens, time >= 2020)) +
  geom_line(aes(time, dist), col="#224B95", alpha=0.2, lwd=1) +
  geom_vline(xintercept=maxt_takens, lty=2, col="#224B95") +
  geom_smooth(data=filter(dist_takens, time>=maxt_takens), aes(time, dist), method="lm", col="#224B95") +
...
ggsave("figure_test_takens_sirs.pdf", gcomb, width=12, height=8)

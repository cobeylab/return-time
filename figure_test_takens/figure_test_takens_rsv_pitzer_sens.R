...
for (i in 1:length(delayvec)) {
  ...
  out <- lapply(2:maxdimension, function(j){
    mat_perturb_takens <- takens(log(ss_perturb$prevalence), d=j, tau=delay)
    mat_unperturb_takens <- takens(log(ss_unperturb$prevalence), d=j, tau=delay)
    ...
    lfit <- lm(log(dist)~time, data=filter(dist_takens, time>=maxt, time<=maxt+1/ee))
...
g1 <- ggplot(resdata) +
  ...
  coord_cartesian(xlim=c(2020, 2030), ylim=c(1e-5, 30)) +
...
ggsave("figure_test_takens_rsv_pitzer_sens.pdf", gcomb, width=15, height=10)

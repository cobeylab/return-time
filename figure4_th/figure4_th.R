...
load("../analysis_takens_acf_fnn_th/analysis_korea_takens_acf_fnn_th.rda")
load("../analysis_takens_acf_fnn_th/analysis_hongkong_takens_acf_fnn_th.rda")
load("../analysis_takens_acf_fnn_th/analysis_hongkong_piv_takens_acf_fnn_th.rda")
load("../analysis_takens_acf_fnn_th/analysis_hongkong_noro_takens_acf_fnn_th.rda")
load("../analysis_takens_acf_fnn_th/analysis_canada_takens_acf_fnn_th.rda")
load("../analysis_takens_acf_fnn_th/analysis_nrevss_takens_acf_fnn_th.rda")

analysis_all <- bind_rows(
  analysis_korea_takens_acf_fnn_th %>%
    mutate(country="Korea",
           time=year+week/52),
  analysis_hongkong_takens_acf_fnn_th  %>%
    mutate(
      country="Hong Kong",
      key=factor(key,
  ...
  analysis_hongkong_piv_takens_acf_fnn_th %>%
    mutate(
      country="Hong Kong",
      key="Parainfluenza virus",
      time=year+week/52
    ),
  analysis_hongkong_noro_takens_acf_fnn_th %>%
    mutate(
      country="Hong Kong",
      time=year+match(month,month.name)/12
    ),
  analysis_canada_takens_acf_fnn_th %>%
    filter(
  ...
  analysis_nrevss_takens_acf_fnn_th %>%
    mutate(
      country="US",
      key=factor(type,
...
analysis_all_summ_filter_th <- analysis_all_summ %>%
  filter(
    reldist < exp(2)
  )
...
ggsave("figure4_dist_th.pdf", g1, width=12, height=6)

measles_resilience <- -eigen_seir()

analysis_all_summ_filter_th %>%
...
lfit <- lm(resilience~country+key, data=analysis_all_summ_filter_th)

summary(lfit)

afit <- aov(resilience~country+key, data=analysis_all_summ_filter_th)

summary(afit)

g2 <- ggplot(analysis_all_summ_filter_th) +
...
g3 <- ggplot(analysis_all_summ_filter_th) +
...
ggsave("figure4_th.pdf", gcomb, width=6, height=6)
save("analysis_all_summ_filter_th", file="figure4_summ_th.rda")

g3b <- ggplot(analysis_all_summ_filter_th) +
  geom_errorbar(aes(key, ymin=when_lwr, ymax=when_upr, col=country), width=0, 
                position = position_dodge(width=0.5)) +
  geom_point(aes(key, when, col=country, shape=country), 
             position = position_dodge(width=0.5), size=3) +

...
for (i in 1:length(name)) {
  print(i)
  load(paste0("../stanfit_hongkong/stanfit_hongkong_growth_tv_", name[i], ".rda"))
  ...
  ss <- get(paste0("stanfit_hongkong_growth_tv_", name[i]))
...
analysis_hongkong_resp_growth <- estlist %>%
  bind_rows

analysis_hongkong_growth_pred <- predlist %>%
  bind_rows

save("analysis_hongkong_resp_growth", "analysis_hongkong_growth_pred", file="analysis_hongkong_resp_growth.rda")

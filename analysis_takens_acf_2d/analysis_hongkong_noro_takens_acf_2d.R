...
distdata_takens <- distfun(
  ...
  time=tail(tmp$year+match(tmp$month,month.name)/12, -tau),
  tbreak=2020,
  out="all"
)
...
analysis_hongkong_noro_takens_acf_2d <- tmp

save("analysis_hongkong_noro_takens_acf_2d", file="analysis_hongkong_noro_takens_acf_2d.rda")

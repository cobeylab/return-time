...
ss_perturb_sirs <- simulate_sirs_import(
  theta=0,
  omega=1e-7,
  tmax=2040) %>%
  filter(time >= 2010) %>%
...
ss_unperturb_sirs <- simulate_sirs_import(theta=0,
                                          omega=1e-7,
...
g2 <- ggplot(bind_cols(as.data.frame(mat_perturb_SI_sirs), data.frame(time=time))) +
  ...
  scale_x_continuous("Susceptibles", limits=c(0.2, 0.55), expand=c(0,0)) +
  scale_y_log10("Infected", limits=c(1e-7, 1e-1), expand=c(0,0)) +
  theme(
    panel.grid = element_blank(),
    legend.position = "none"
  )
...
ss_perturb_sirs2 <- simulate_sirs_import(
  theta=0,
  omega=1e-6,
  tmax=2040) %>%
  filter(time >= 2010) %>%
...
ss_unperturb_sirs2 <- simulate_sirs_import(theta=0,
                                          omega=1e-6,
                                          tmax=2040,npifun=function(x)1) %>%
  filter(time >= 2010)

time <- ss_perturb_sirs2$time
...
g4 <- ggplot(ss_perturb_sirs2) +
  ...
  scale_y_continuous("Infected", expand=c(0, 0), limits=c(0, 0.13)) +
  scale_color_manual(values=c("#224B95", "#EF6351")) +
  ggtitle("Intermediate importation") +
  theme(
    panel.grid = element_blank(),
    legend.position = "none"
  )
...
g6 <- ggplot(filter(dist_SI_sirs2, time >= 2020)) +
  geom_line(aes(time, dist), col="#224B95", lwd=1) +
  geom_smooth(aes(time, dist), method="loess", col="orange") +
  geom_function(fun=function(x) exp(predict(lfit_SI_sirs2)[1]) * exp(-ee_sirs*(x-maxt_SI_sirs)),
                lwd=2, lty=3) +
  scale_x_continuous("Year", expand=c(0, 0), limits=c(NA, 2029.5),
                     breaks=seq(2020, 2030, by=2)) +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
...
ss_perturb_sirs3 <- simulate_sirs_import(
  theta=0,
  omega=1e-5,
  tmax=2040) %>%
  filter(time >= 2010) %>%
...
ss_unperturb_sirs3 <- simulate_sirs_import(theta=0,
                                           omega=1e-5,
                                           tmax=2040,npifun=function(x)1) %>%
  filter(time >= 2010)

time <- ss_perturb_sirs3$time
...
g9 <- ggplot(filter(dist_SI_sirs3, time >= 2020)) +
  geom_line(aes(time, dist), col="#224B95", lwd=1) +
  geom_smooth(aes(time, dist), method="loess", col="orange") +
  geom_function(fun=function(x) exp(predict(lfit_SI_sirs3)[1]) * exp(-ee_sirs*(x-maxt_SI_sirs)),
                lwd=2, lty=3) +
  scale_x_continuous("Year", expand=c(0, 0), limits=c(NA, 2029.5),
                     breaks=seq(2020, 2030, by=2)) +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
...
ss_perturb_sirs4 <- simulate_sirs_import(
  theta=0,
  omega=1e-4,
  tmax=2040) %>%
  filter(time >= 2010) %>%
...
ss_unperturb_sirs4 <- simulate_sirs_import(theta=0,
                                           omega=1e-4,
                                           tmax=2040,npifun=function(x)1) %>%
  filter(time >= 2010)

time <- ss_perturb_sirs4$time

mat_perturb_SI_sirs4 <- matrix(c(ss_perturb_sirs4$S, log(ss_perturb_sirs4$I)), ncol=2)
mat_unperturb_SI_sirs4 <- matrix(c(ss_unperturb_sirs4$S, log(ss_unperturb_sirs4$I)), ncol=2)

dist_SI_sirs4 <- distfun(mat_perturb_SI_sirs4, mat_unperturb_SI_sirs4,
                         ss_perturb_sirs4$time,
                         2020, "all",
                         scale=FALSE)

g10 <- ggplot(ss_perturb_sirs4) +
  annotate("rect", xmin=2020, xmax=2020.5, ymin=-Inf, ymax=Inf, fill="gray80") +
  geom_line(aes(time, I, col=group)) +
  geom_line(data=ss_unperturb_sirs, aes(time, I), lty=2, col="#EF6351") +
  scale_x_continuous("Year", expand=c(0, 0), limits=c(NA, 2029.5)) +
  scale_y_continuous("Infected", expand=c(0, 0), limits=c(0, 0.13)) +
  scale_color_manual(values=c("#224B95", "#EF6351")) +
  ggtitle("Very high importation") +
  theme(
    panel.grid = element_blank(),
    legend.position = "none"
  )

g11 <- ggplot(bind_cols(as.data.frame(mat_perturb_SI_sirs4), data.frame(time=time))) +
  geom_path(aes(V1, exp(V2)), col="#224B95") +
  geom_path(data=bind_cols(as.data.frame(mat_unperturb_SI_sirs4), data.frame(time=time)),
            aes(V1, exp(V2)), col="#EF6351") +
  scale_x_continuous("Susceptibles", limits=c(0.2, 0.55), expand=c(0,0)) +
  scale_y_log10("Infected", limits=c(1e-7, 1e-1), expand=c(0,0)) +
  theme(
...
maxt_SI_sirs4 <- dist_SI_sirs4$time[which.max(dist_SI_sirs4$dist)]
lfit_SI_sirs4 <- lm(log(dist)~time, data=filter(dist_SI_sirs4, time>=maxt_SI_sirs4))

g12 <- ggplot(filter(dist_SI_sirs4, time >= 2020)) +
  geom_line(aes(time, dist), col="#224B95", lwd=1) +
  geom_smooth(aes(time, dist), method="loess", col="orange") +
  geom_function(fun=function(x) exp(predict(lfit_SI_sirs4)[1]) * exp(-ee_sirs*(x-maxt_SI_sirs)),
                lwd=2, lty=3) +
  scale_x_continuous("Year", expand=c(0, 0), limits=c(NA, 2029.5),
                     breaks=seq(2020, 2030, by=2)) +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
...
ggsave("figure2_simple_noseas_import.pdf", gcomb, width=12, height=8)

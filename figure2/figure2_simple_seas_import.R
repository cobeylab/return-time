...
ss_unperturb_sirs2 <- simulate_sirs_import(theta=0.2,
                                           tmax=2040,npifun=function(x)1) %>%
  filter(time >= 2010)
...
ss_unperturb_sirs3 <- simulate_sirs_import(theta=0.4,
                                    tmax=2040,npifun=function(x)1) %>%
  filter(time >= 2010)
...
ggsave("figure2_simple_seas_import.pdf", gcomb, width=10, height=8)

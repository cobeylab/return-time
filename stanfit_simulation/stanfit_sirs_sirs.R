...
npiend <- which(data_fit$year==2021 & data_fit$week==13)
...
stanfit_sirs_sirs <- sampling(model,
                             data = standata,
                             seed=103,
                             chain=4,
                             cores=4,
...
check_hmc_diagnostics(stanfit_sirs_sirs)
get_num_divergent(stanfit_sirs_sirs)
get_num_max_treedepth(stanfit_sirs_sirs)
get_low_bfmi_chains(stanfit_sirs_sirs)
get_bfmi(stanfit_sirs_sirs)

save("stanfit_sirs_sirs", file="stanfit_sirs_sirs.rda")

ss <- summary(stanfit_sirs_sirs)
...
plot(ss$summary[grepl("beta\\[", rownames(ss$summary)),6], type="l", ylim=c(0, 10))

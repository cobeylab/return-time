...
In theory, we can now quantify how fast this distance decreases }\DIFdelend %DIF >  Specifically, the nearest neighbor distance is calculated by computing the distance between the current position on the $M$-dimensional space and all points in the empirical attractor set and taking the minimum value.
\DIFaddbegin \DIFadd{In theory, the rate at which the distance from this attractor decreases, which can be estimated }\DIFaddend by fitting a linear regression on a log scale, \DIFdelbegin \DIFdel{where the slope of the linear regression empirically measures pathogen resilience , with a steeper slope corresponding to a higher resilience estimate }\DIFdelend \DIFaddbegin \DIFadd{provides an empirical measure of pathogen resilience }\DIFaddend (Figure 3E).
...
%DIF >  We note that the interplay between seasonal transmission and intrinsic periodicity can also lead to complex cycles, as illustrated by a recent analysis of \textit{Mycoplasma pneumoniae} dynamics \citep{nielsen2025complex}.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{We cannot rule out the possibility that potential long-term impacts of pandemic perturbations may be caused by changes in testing.
}
...
\begin{equation}
-\DIFdelbegin \DIFdel{\frac{\mathrm{Re}(\lambda)}{2} }\DIFdelend \DIFaddbegin \DIFadd{\mathrm{Re}(\lambda) }\DIFaddend = \frac{\mu}{2 S^\ast} = \frac{\tsub{S}{replenish}}{2}.
\end{equation}

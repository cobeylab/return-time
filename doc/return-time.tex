...
Moreover, the possibility of a long-lasting impact of the pandemic NPIs pose an important question for future infectious disease dynamics: can we predict whether and when other respiratory pathogens will eventually return to their pre-pandemic dynamics?
...
We begin by laying out a few representative scenarios that capture the potential impact of COVID-19 interventions on endemic pathogen dynamics and illustrate how resilience can be measured by comparing the pre- and post-pandemic dynamics of susceptible and infected hosts.
...
(B, E, H) Phase plane representation of the epidemic dynamics from corresponding models.
...
For example, a stage-structured model initially exhibits a stable annual cycle, but perturbations from NPIs cause the epidemic to shift to biennial cycles (Figure 2G).
...
We also explore measuring the resilience of a two-strain host-pathogen system:
...
Simulations from a simple two-strain system illustrate that separate analyses of individual strain dynamics (e.g., RSV A vs B) and a joint analysis of total infections (e.g., total RSV infections) (Supplementary Figure S2, 3). 
\swp{This is expected because the phase portaits of each of the coupled strains are projections of the shared attractor, according to Takens Theorem. (Is this precisely correct?)}
...
using longer delays and higher dimensions tends to smooth out temporal variations in the distance from attractor (Supplementary Figure S6).
...
However, these interventions offer large-scale natural experiments for understanding how different pathogens respond to perturbations.
...
As a reference, we also included time series data on norovirus infections for available countries---in contrast to respiratory pathogens, we expect gastrointestinal viruses, such as norovirus, to be differently affected by COVID-19 intervention measures.
...
The basic reproduction number $\mathcal R_0(t) = \beta(t)/(\gamma + \mu)$ is defined as the average number of secondary infections caused by a single infected individual in a fully susceptible population and measures the intrinsic transmissibility of a pathogen.

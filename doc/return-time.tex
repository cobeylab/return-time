...
To do so, we start with an embedding dimension $e$ and construct a set of points $X_{\tau,e}$.
Then, for each point $X_{\tau,e}(t)$, we determine the nearest neighbor from the set $X_{\tau,e}$, which we denote $X_{\tau,e}(\tsub{t}{nn})$ for $t \neq \tsub{t}{nn}$.
...
To determine the appropriate embedding dimension, we choose a threshold $R$

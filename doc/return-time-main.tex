...
COVID-19 interventions slowed the transmission of many respiratory pathogens in different ways, raising questions about mechanisms driving the variation in responses to intervention measures.
...
Finally, we showed that the speed of replenishment of the susceptible population---for example, through waning immunity---largely determines this sensitivity, suggesting that persistence of common respiratory pathogens is likely driven by rapid susceptible replenishment.
...
These differences likely reflect variation in intervention patterns, pathogen characteristics, immigration/importation from other countries, and pre-pandemic pathogen dynamics \citep{perofsky2024impacts}.
Therefore, comparing the differential impact of the pandemic perturbations across pathogens can provide unique opportunities to learn about underlying pathogen characteristics across different populations, such as their transmissibility or duration of immunity \citep{chow2023effects}.
...
While quantifying this timing is particularly useful for public health agencies as it allows them to prepare for future outbreaks, epidemic rebound differs from the recovery of the underlying dynamics---the rebound marks the initial return of pathogen activity without providing any information about the subsequent dynamics.
Instead, we seek to characterize how fast (and whether) a pathogen returns to its pre-pandemic dynamics by utilizing all of dynamical information following the perturbation, rather than simply focusing on the initial rebound.
These two concepts have a subtle but important difference. 
For example, it took more than 3 years for human metapneumovirus to rebound in Hong Kong, but the observed epidemic patterns in 2024 appear similar to the pre-pandemic seasonal mean, suggesting a potential, rapid return to pre-pandemic dynamics within 2 years (Figure 1).
...
In practice, we rarely know the true, underlying data generating process that describes the dynamics of common respiratory pathogens, which can depend on many hidden variables, such as changes in population-level susceptibility.
Instead, we have to rely on simplifying model assumptions, raising questions about whether we can infer the intrinsic resilience of a system without measurements of all variables that describe the observed dynamics.
...
As we show later, this measure of resilience depends on the rate of susceptible replenishment, where faster replenishment (e.g., through faster waning immunity) causes a pathogen to be more resilient, and further determines how the system responds to small, stochastic perturbations.
Therefore, quantifying pathogen resilience provides key information about their dynamical characteristics.
...
To quantify resilience, we measure the distance from the pre-pandemic attractor---a set of points in state space or phase plane that the system is pulled towards \citep{hastings1993chaos}---by taking Euclidean distance on the SI phase plane relative to the counterfactual unperturbed trajectory (Figure 2B; Materials and Methods).
This distance decreases exponentially with time on average, as demonstrated by the locally estimated scatterplot smoothing (LOESS) fit, and the overall rate of return matches the intrinsic resilience of the seasonally unforced system (Figure 2C).
...
For example, if a 10\% reduction in transmission persists even after the major pandemic perturbations are lifted, the system will converge to a new attractor (Figure 2D--F), but we may not be able to confirm the convergence to a new attractor in practice until many years have passed.
Nonetheless, we can still measure the distance from the pre-pandemic attractor (Figure 2E), which initially decreases exponentially on average (equivalently, linearly on a log scale) and eventually plateaus (Figure 2F).
...
For example, a stage-structured model that accounts for reduction in secondary susceptibility initially exhibits a stable annual cycle, but perturbations from a 50\% reduction in transmission for 6 months cause the epidemic to shift to a transient biennial cycle before it eventually approaches the original pre-pandemic attractor (Figure 2G,H).
As indicated by the LOESS fit, the distance from the attractor initially decreases exponentially at a rate that is consistent with the intrinsic resilience of the seasonally unforced stage-structured system, but the rate of return slows down with the damped oscillations (Figure 2I).
...
However, we note that a small amount of importation of infections can cause this transient to disappear and cause the system to become much more resilient, even though it has negligible effect on overall dynamics or their empirical resilience under non-seasonal or moderately seasonal transmission (Supplementary Figure S2);
under non-seasonal transmission, importation has limited effect on overall resilience of the system unless the amount of importation is increased to a unrealistically high value (Supplementary Figure S3).
...
Simulations from a simple two-strain competition system illustrate that separate analyses of individual strain dynamics (e.g., RSV subtype A vs B) and a joint analysis of total infections (e.g., total RSV infections) yield identical resilience estimates (Supplementary Figure S4, 5).
...
We first tested this approach against simulations and analyzed case time series of respiratory pathogens from four countries: Canada, Hong Kong, Korea, and the US.
...
For example, continued transmission reduction from interventions can limit the size of the first outbreak following the rebound, allowing for a larger outbreak in when interventions are further relaxed (Figure 3B).
...
In addition, uncertainties in case counts due to observation error, strain evolution, and multiannual cycles in the observed epidemic dynamics (e.g., adenovirus circulation patterns in Hong Kong and Korea) add challenges to defining pre-pandemic attractors.
...
Specifically, by taking the logged values of pre-pandemic cases $\log(C(t)+1)$ and creating delayed copies, Takens' theorem allows us to define the pre-pandemic attractor as a points on an $M$-dimensional space:
\begin{equation}
...
Intuitively, the time series of case counts is generated from a complex system that depends on many hidden variables, such as changes in susceptibility. 
Takens’ theorem tells us that, delayed copies of the observed series preserves the dynamics of the underlying multi-dimensional attractor.
We can then apply the same delay and embedding dimensions to the entire time series to determine the position in multi-dimensional state space (Figure 3D) and measure the nearest neighbor distance between the current state of the system and the empirical pre-pandemic attractor (Figure 3E).
...
In theory, the rate at which the distance from this attractor decreases, which can be estimated by fitting a linear regression on a log scale, provides an empirical measure of pathogen resilience (Figure 3E).
However, resulting estimates of pathogen resilience can be sensitive to choices about embedding delays and dimensions, where longer delays and higher dimensions tends to smooth out temporal variations in the distance from the attractor (Supplementary Figure S8).
...
We note that estimating pathogen resilience from linear regression is sensitive to choices about regression fitting windows (Figure 3E).
Therefore, we developed an automated window selection criteria for linear regression and tested it against randomized, stochastic simulations across a range of realistic pandemic perturbation shapes.
...
We then applied the best-performing window selection criteria and embedding dimension to pathogen surveillance data presented in Figure 1 (Materials and Methods).
...
The window selection criteria gave poor regression windows in three cases (norovirus in Hong Kong and Korea and rhinovirus/enterovirus in the US), leading to unrealistically low resilience estimates, and so we used ad-hoc regression windows instead (Supplementary Figure S11; Materials and Methods).
...
In contrast, pathogens that were predicted to have not returned yet also showed clear differences from their pre-pandemic seasonal averages; as many of these pathogens are predicted to return in 2025--2026, we may be able to test these predictions in near future (Supplementary Figure S12).
...
We also evaluated the ability of our approach to make out-of-sample predictions by using observations up to the 26th week of 2023 (Supplementary Figure S15--16).
...
An increase in $\mathcal R_0$ and a decrease in the duration of immunity leads to a higher resilience value (Figure 5A) and a faster per-capita rate of susceptible replenishment, the ratio between total rate at which new susceptibles enter the population and the equilibrium number of susceptibles, $S^\ast$ (Figure 5B).
Specifically, a higher $\mathcal R_0$ drives a faster per-capita susceptible replenishment rate by decreasing $S^\ast$.
For a simple SIR model that assumes a life-long immunity, pathogen resilience is proportional to the per-capita rate of susceptible replenishment (Materials and Methods).
...
In contrast, there was a large uncertainty in the estimates for susceptible replenishment rates due to a lack of one-to-one correspondence between susceptible replenishment rates and pathogen resilience (Figure 5B).
...
Here, a seasonally unforced SIRS model with constant birth and death rates illustrates that resilience of a host-pathogen system determines the sensitivity to perturbations caused by demographic stochasticity, where a less resilient system experiences a greater deviation from the deterministic trajectory (Figure 6, Materials and Methods).
Less resilient systems also exhibited slower epidemic cycles (Figure 6A--C). 
The periodicity of this epidemic cycle matched those predicted by the intrinsic periodicity of the system (Supplementary Figure S17), which is inversely proportional to the intrinsic resilience (Supplementary Figure S18).
We note that the interplay between seasonal transmission and intrinsic periodicity can also lead to complex cycles, as illustrated by a recent analysis of \textit{Mycoplasma pneumoniae} dynamics \citep{nielsen2025complex}.
...
These changes may reflect changes in surveillance or actual shift in the dynamics.
...
There are many other factors that may affect the resilience of a host-pathogen system beyond susceptible replenishment rate.
For example, we showed that importation can also increase the resilience of the system (Supplementary Figures S2, 3).
Interestingly, this effect is minimal without seasonal forcing but substantially amplified under strong seasonal forcing, suggesting that analyzing the eigenvalues of seasonally unforced system may be insufficient to capture key aspects of the system’s resilience.
Future studies should consider both analytical and numerical analyses of more detailed, mechanistic models to understand how other factors---such as the generation-interval distribution \citep{lloyd2001realistic}, immune boosting \citep{lavine2011natural}, strain competition \citep{reich2013interactions}, and post-infection mortality \citep{saad2023epidemiological}---may alter the resilience of a host-pathogen system.
...
We tried to account for changes in testing patterns by either scaling the case counts (for Canada, Hong Kong, and Korea) or deriving incidence proxy (for the US).
We were unable to derive incidence proxy for Canada and Hong Kong due to instability of testing positivity, including long-term reduction in test positivity associated with an increase in testing volume.
Consistency in resilience estimates across different countries provide some robustness to our conclusions, our exact estimates are likely sensitive to changes in testing.
We also cannot rule out the possibility that potential long-term impacts of pandemic perturbations may simply reflect changes in testing, rather than actual dynamical changes.
Future studies should investigate improved methods for accounting for changes in testing patterns to obtain a more accurate perspective on the differences between current and past outbreak patterns of endemic pathogens.
...
When we include importation in Supplementary Figures S2 and S3, we model the force of infection as $\beta(t) (I + \omega)$ instead and vary $\omega$ between $10^{-7}$ and $10^{-4}$ per day.
...
\begin{equation}
-\mathrm{Re}(\lambda) = \frac{\mu}{2 S^\ast} = \frac{\tsub{S}{replenish}}{2}.
\end{equation}

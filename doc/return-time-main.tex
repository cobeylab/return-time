...
Specifically, by taking the logged values of pre-pandemic cases $\log(C(t)+1)$ and creating delayed copies, Takens' theorem allows us to define the pre-pandemic attractor as a points in an $M$-dimensional space:
\begin{equation}
...
Estimating pathogen resilience from linear regression is sensitive to choices about regression fitting windows (Figure 3E).
Therefore, we developed an automated window selection criterion for linear regression and tested it against randomized, stochastic simulations across a range of realistic pandemic perturbation shapes.
...
We then applied the best-performing window selection criterion and embedding dimension to pathogen surveillance data presented in Figure 1 (Materials and Methods).
...
Here, a seasonally unforced SIRS model with constant birth and death rates illustrates that resilience of a host-pathogen system determines the sensitivity to perturbations caused by demographic stochasticity (Figure 6, Materials and Methods).
Less resilient systems exhibits greater deviations from the deterministic trajectory and slower epidemic cycles (Figure 6A--C). 
...
The intrinsic resilience is not the sole determinant for of a system's sensitivity to stochastic perturbations.
...
Many other factors may affect the resilience of a host-pathogen system beyond susceptible replenishment rate.
...
We cannot rule out the possibility that potential long-term impacts of pandemic perturbations may be caused by changes in testing.
...
In particular, quantifying the resilience of a host-pathogen system offers insight into endemic pathogens' responses to pandemic perturbations, including whether some pathogens will exhibit long-lasting impact from the pandemic perturbation or not.

...
g2 <- ggplot(hongkong_pred) +
  ...
  geom_ribbon(aes(year+week/52, ymin=lwr, ymax=upr), fill="gray", col="gray90", alpha=0.3, lwd=0.5) +
  geom_line(aes(year+week/52, mean), col="gray", lwd=0.7) +
  geom_line(data=filter(hongkong_pred, year<2020), aes(year+week/52, cases), col="#EF6351", lwd=0.5) +
  geom_point(data=filter(hongkong_pred, year<2020), aes(year+week/52, cases), col="#EF6351", size=0.7) +
  geom_line(data=filter(hongkong_pred, year>=2020), aes(year+week/52, cases), col="#224B95", lwd=0.5) +
  geom_point(data=filter(hongkong_pred, year>=2020), aes(year+week/52, cases), col="#224B95", size=0.7) +
  scale_x_continuous("Year", expand=c(0, 0),
                     breaks=2013:2024) +
  scale_y_continuous("Scaled cases", expand=c(0, 0)) +
...
ggsave("figure_data.pdf", gcomb, width=12, height=8)

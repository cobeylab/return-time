...
mat_perturb_RI <- matrix(c(ss_perturb$Reff, log(ss_perturb$prevalence)), ncol=2)
mat_unperturb_RI <- matrix(c(ss_unperturb$Reff, log(ss_unperturb$prevalence)), ncol=2)

dist_RI <- distfun(mat_perturb_RI, mat_unperturb_RI,
                   ss_perturb$time,
                   2020, "all")

mat_perturb_rI <- matrix(c(ss_perturb$r, log(ss_perturb$prevalence)), ncol=2)
mat_unperturb_rI <- matrix(c(ss_unperturb$r, log(ss_unperturb$prevalence)), ncol=2)
...
lfit_RI <- lm(log(dist)~time, data=filter(dist_RI, time>=maxt_RI, time<=maxt_RI+1/ee))

g4 <- ggplot(filter(dist_RI, time >= 2020)) +
  geom_line(aes(time, dist), col="#224B95", alpha=0.2, lwd=1) +
  geom_vline(xintercept=maxt_RI, lty=2, col="#224B95") +
  geom_vline(xintercept=maxt_RI+1/ee, lty=2, col="#224B95") +
  geom_smooth(data=filter(dist_RI, time>=maxt_RI, time<=maxt_RI+1/ee), aes(time, dist), method="lm", col="#224B95") +
  geom_smooth(data=filter(dist_RI, time>=maxt_RI+1/ee), aes(time, dist), method="lm", col="#224B95") +
  geom_smooth(data=filter(dist_RI, time>=maxt_RI), aes(time, dist), col="orange") +
...
ggsave("figure_test_rsv_pitzer.pdf", gcomb, width=12, height=8)

...
lfit_RI1 <- lm(log(dist)~time, data=filter(dist_RI1, time>=maxt_RI1))
...
g4 <- ggplot(filter(dist_RI1, time >= 2020)) +
  ...
  geom_function(fun=function(x) exp(predict(lfit_RI1)[1]) * exp(-ee1*(x-maxt_RI1)),
                lwd=0.7, lty=3) +
  geom_smooth(data=filter(dist_RI1, time>=maxt_RI1), aes(time, dist), method="lm", col="#224B95") +
  scale_x_continuous("Year") +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
  coord_cartesian(ylim=c(1e-5, 10)) +
  theme(
...
lfit_RI2 <- lm(log(dist)~time, data=filter(dist_RI2, time>=maxt_RI2))
...
g10 <- ggplot(filter(dist_RI2, time >= 2020)) +
  ...
  geom_function(fun=function(x) exp(predict(lfit_RI2)[1]) * exp(-ee2*(x-maxt_RI2)),
                lwd=0.7, lty=3) +
  geom_smooth(data=filter(dist_RI2, time>=maxt_RI2), aes(time, dist), method="lm", col="#224B95") +
  scale_x_continuous("Year") +
  scale_y_log10("Distance from attractor", expand=c(0, 0)) +
  coord_cartesian(ylim=c(1e-5, 10)) +
  theme(
...
ggsave("figure_test_bhattacharyya.pdf", gcomb, width=12, height=16)
